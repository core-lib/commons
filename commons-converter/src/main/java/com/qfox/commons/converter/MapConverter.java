...
public class MapConverter implements Converter {
	...
	public Object convert(Object source, String name, Class<?> _class, TypeConverter context) throws Exception {
		...
		for (Entry<?, ?> entry : sources.entrySet()) {
			...
			if (context.isExcluded(_name)) {
				continue;
			}
			map.put(context.doConvert(key, _name, key.getClass()), context.doConvert(value, _name, value.getClass()));
		}
	...
	public Object convert(Object source, String name, ParameterizedType type, TypeConverter context) throws Exception {
		...
		for (Entry<?, ?> entry : sources.entrySet()) {
			...
			map.put(context.doConvert(key, _name, type.getActualTypeArguments()[0]), context.doConvert(value, _name, type.getActualTypeArguments()[1]));
		}
		if (_class.isInterface()) {
			_class = implementations.get(_class);
		}
