...
public class ComplexConverter implements Converter {
	...
	public Object convert(Object source, String name, Class<?> _class, TypeConverter context) throws Exception {
		...
		for (PropertyDescriptor descriptor : descriptors) {
			...
			if (descriptor.getName().equals("class") || context.isExcluded(_name)) {
				continue;
			}
			try {
				...
				Object value = context.doConvert(_source, _name, replace(type, maping));
	...
	public Object convert(Object source, String name, ParameterizedType type, TypeConverter context) throws Exception {
		...
		for (PropertyDescriptor descriptor : descriptors) {
			...
			try {
				...
				Object value = context.doConvert(_source, _name, replace(_type, maping));
				descriptor.getWriteMethod().invoke(result, value);
			} catch (IntrospectionException e) {
				continue;
			}
