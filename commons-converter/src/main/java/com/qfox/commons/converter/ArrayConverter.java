...
public class ArrayConverter implements Converter {
	...
	public Object convert(Object source, String name, Class<?> _class, TypeConverter context) throws Exception {
		...
		if (_class.getComponentType().isPrimitive()) {
			...
			for (int i = 0; i < length; i++) {
				Array.set(target, i, context.doConvert(Array.get(source, i), name, _class.getComponentType()));
			}
		}
		return target;
	}
	...
	public Object convert(Object source, String name, GenericArrayType type, TypeConverter context) throws Exception {
		...
		for (int i = 0; i < length; i++) {
			Array.set(target, i, context.doConvert(Array.get(source, i), name, type.getGenericComponentType()));
		}
		return target;
	}
